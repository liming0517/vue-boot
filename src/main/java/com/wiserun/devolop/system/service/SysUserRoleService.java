...
import com.wiserun.devolop.system.dao.SysUserRoleDao;
import com.wiserun.devolop.system.entity.SysRoleinfo;
import com.wiserun.devolop.system.entity.SysUserRole;
import com.wiserun.devolop.system.entity.SysUserRoleSch;
import com.wiserun.devolop.system.service.SysUserRoleService;//用于读取代码表数据
...
@Service
public class SysUserRoleService extends CrudService<SysUserRole, SysUserRoleDao> {
	 ...
	 * 无业务需求则可不做任何处理 
	 */
	public void setInfoByID(SysUserRole entity) {
		
		 //根据实际需要扩展--如前台已传入相应数据，这里也可以不做任何处理，以提高执行效率
		SysUserRole sysUserRole;
		if(null!=entity.getId()){
			sysUserRole=sysUserRoleservice.get(entity.getId());
			
			if (null!=sysUserRole){
	           //entity.setProjectTypeCode(sysUserRole.getDetailCode()); //项目代码
	           //entity.setProjectTypeName(sysUserRole.getCodeName()); //项目名称
			}
		}
	 ...
	 * 无业务需求则可不做任何处理 
	 */
	public void setInfoByName(SysUserRole entity) {
	 ...
	 * @param entity
	 */
	public ExpressionQuery queryNoLimitBefore(SysUserRoleSch entity){
	 ...
	 * @param entity
	 */
	public ExpressionQuery queryBefore(SysUserRoleSch entity){
		ExpressionQuery query = new ExpressionQuery();
		query=this.buildExpressionQuery(entity);
	 ...
	 */
	public MessageResult insertCheck(SysUserRole entity){
	 ...
	 * @return MessageResult 如 MessageResult.isError 则表示判断失败
	 */
	public MessageResult updateCheck(SysUserRole entity){
	 ...
	 * @return MessageResult 如 MessageResult.isError 则表示判断失败
	 */
	public MessageResult delCheck(SysUserRole entity){
		DefaultMessageResult mes; //消息对象
	 ...
	 * @return MessageResult 如 MessageResult.isError 则表示判断失败
	 */
	public MessageResult lgdelCheck(SysUserRole entity){
	 ...
	 * @param entity
	 */
	public void insertOrUpdateBefore(SysUserRole entity){
	 ...
	 * @param entity
	 */
	public void lgdelBefore(SysUserRole entity){
	 ...
	 * @param entity
	 */
	public void delBefore(SysUserRole entity){
	 ...
	 */
	public void insertOrUpdateBatchBefore(List<SysUserRole> EntiyList){
	 ...
	 */
	public void lgdelBatchBefore(List<SysUserRole> EntiyList){
		if (null==EntiyList) return ; //为空则跳出
	 ...
	 */
	public void delBatchBefore(List<SysUserRole> EntiyList){
		if (null==EntiyList) return ; //为空则跳出
	 ...
	 * @param entity
	 */
	public void insertOrUpdateAfter(SysUserRole entity){
	 ...
	 * @param entity
	 */
	public void lgdelAfter(SysUserRole entity){
		if (null==entity) return ; //为空则跳出
	 ...
	 * @param entity
	 */
	public void delAfter(SysUserRole entity){
		if (null==entity) return ; //为空则跳出
	 ...
	 * @param List<Entity> --批量（新增或更新）对象
	 */
	public void insertOrUpdateBatchAfter(List<SysUserRole> EntiyList){
	 ...
	 * @param List<Entity> --批量（逻辑删除）对象
	 */
	public void lgdelBatchAfter(List<SysUserRole> EntiyList){
	 ...
	 * @param List<Entity> --批量（物理删除）对象
	 */
	public void delBatchAfter(List<SysUserRole> EntiyList){
	...
	public MessageResult batchByCheckSysUserRole(Map<String, Object> map) throws RuntimeException {
		...
		////////////主表对象-map.get 获取的form名需要与前台保持一致//////////////
		 ...
		 SysUserRoleSch entity=JSON.parseObject((String)(map.get("schForm")), SysUserRoleSch.class); //form名称：schForm 由前台页面指定
		 ...
		 */
		 ...
		 ////////////明细表格对象 dg//////////////
	   	 ...
	   	 List<SysUserRole> entityInserted=JSON.parseArray((String)jsondg.get("inserted"), SysUserRole.class);
	   	 List<SysUserRole> entityUpdated=JSON.parseArray((String)jsondg.get("updated"), SysUserRole.class);
	 	 List<SysUserRole> entitylgDeleted=JSON.parseArray((String)jsondg.get("deleted"), SysUserRole.class);
	...
	public List<SysUserRole> getUserRole(String userId){
		...
		expres.add(new InnerJoinExpression("sys_user_role", "t2", "id", "roleId"));
		expres.add(new ValueExpression("t2.userId", userId));
		
		ExpressionQuery query = ExpressionQuery.buildQueryAll();
		query.addAll(expres);
		
		return sysUserRoleservice.find(query);
	}

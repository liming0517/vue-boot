...
import com.wiserun.devolop.system.dao.SysUserinfoDao;
import com.wiserun.devolop.system.entity.SysUserRole;
import com.wiserun.devolop.system.entity.SysUserinfo;
import com.wiserun.devolop.system.entity.SysUserinfoSch;
import com.wiserun.devolop.system.service.SysUserinfoService;//用于读取代码表数据
...
@Service
public class SysUserinfoService extends CrudService<SysUserinfo, SysUserinfoDao> {
	 ...
	 * 无业务需求则可不做任何处理 
	 */
	public void setInfoByID(SysUserinfo entity) {
		
		 //根据实际需要扩展--如前台已传入相应数据，这里也可以不做任何处理，以提高执行效率
		SysUserinfo sysUserinfo;
		if(null!=entity.getId()){
			sysUserinfo=sysUserinfoservice.get(entity.getId());
			
			if (null!=sysUserinfo){
	           //entity.setProjectTypeCode(sysUserinfo.getDetailCode()); //项目代码
	           //entity.setProjectTypeName(sysUserinfo.getCodeName()); //项目名称
			}
		}
	 ...
	 * 无业务需求则可不做任何处理 
	 */
	public void setInfoByName(SysUserinfo entity) {
	 ...
	 * @param entity
	 */
	public ExpressionQuery queryNoLimitBefore(SysUserinfoSch entity){
	 ...
	 * @param entity
	 */
	public ExpressionQuery queryBefore(SysUserinfoSch entity){
		ExpressionQuery query = new ExpressionQuery();
		query=this.buildExpressionQuery(entity);
	 ...
	 */
	public MessageResult insertCheck(SysUserinfo entity){
	 ...
	 * @return MessageResult 如 MessageResult.isError 则表示判断失败
	 */
	public MessageResult updateCheck(SysUserinfo entity){
	 ...
	 * @return MessageResult 如 MessageResult.isError 则表示判断失败
	 */
	public MessageResult delCheck(SysUserinfo entity){
		DefaultMessageResult mes; //消息对象
	 ...
	 * @return MessageResult 如 MessageResult.isError 则表示判断失败
	 */
	public MessageResult lgdelCheck(SysUserinfo entity){
	 ...
	 * @param entity
	 */
	public void insertOrUpdateBefore(SysUserinfo entity){
	 ...
	 * @param entity
	 */
	public void lgdelBefore(SysUserinfo entity){
	 ...
	 * @param entity
	 */
	public void delBefore(SysUserinfo entity){
	 ...
	 */
	public void insertOrUpdateBatchBefore(List<SysUserinfo> EntiyList){
	 ...
	 */
	public void lgdelBatchBefore(List<SysUserinfo> EntiyList){
		if (null==EntiyList) return ; //为空则跳出
	 ...
	 */
	public void delBatchBefore(List<SysUserinfo> EntiyList){
		if (null==EntiyList) return ; //为空则跳出
	 ...
	 * @param entity
	 */
	public void insertOrUpdateAfter(SysUserinfo entity){
	 ...
	 * @param entity
	 */
	public void lgdelAfter(SysUserinfo entity){
		if (null==entity) return ; //为空则跳出
	 ...
	 * @param entity
	 */
	public void delAfter(SysUserinfo entity){
		if (null==entity) return ; //为空则跳出
	 ...
	 * @param List<Entity> --批量（新增或更新）对象
	 */
	public void insertOrUpdateBatchAfter(List<SysUserinfo> EntiyList){
	 ...
	 * @param List<Entity> --批量（逻辑删除）对象
	 */
	public void lgdelBatchAfter(List<SysUserinfo> EntiyList){
	 ...
	 * @param List<Entity> --批量（物理删除）对象
	 */
	public void delBatchAfter(List<SysUserinfo> EntiyList){
	...
	public MessageResult batchByCheckSysUserinfo(Map<String, Object> map) throws RuntimeException {
		...
		////////////主表对象-map.get 获取的form名需要与前台保持一致//////////////
		 ...
		 SysUserinfoSch entity=JSON.parseObject((String)(map.get("schForm")), SysUserinfoSch.class); //form名称：schForm 由前台页面指定
		 ...
		 */
		 ...
		 ////////////明细表格对象 dg//////////////
	   	 ...
	   	 List<SysUserinfo> entityInserted=JSON.parseArray((String)jsondg.get("inserted"), SysUserinfo.class);
	   	 List<SysUserinfo> entityUpdated=JSON.parseArray((String)jsondg.get("updated"), SysUserinfo.class);
	 	 List<SysUserinfo> entitylgDeleted=JSON.parseArray((String)jsondg.get("deleted"), SysUserinfo.class);
	...
	public SysUserinfo getByUsername(String username) {
		QBC<SysUserinfo> qbc = QBC.create(this.getDao());
	 ...
	 * @return 返回明文密码
	 */
	public String resetUserPassword(SysUserinfo user){
		
		String password = createNewPswd();
		
		String hash = PasswordUtil.createStorePswd(password);
		
		SysUserinfo record = this.get(user);
	...
	public void updateUserPassword(SysUserinfo user,String md5Pswd){    	
		user.setPassword(PasswordUtil.createHash(md5Pswd));    	
    	update(user);
	}
	
	//public void optState(int userId,byte destState) {
	public void optState(String userId,String destState) {
		List<SysUserRole> sysUserRole = sysUserRoleService.getUserRole(userId);
		if(CollectionUtils.isNotEmpty(sysUserRole)){
			String  adminRoleId = RMSContext.getInstance().getAdminRoleId();
			for (SysUserRole role : sysUserRole) {
				if(adminRoleId == role.getId()) {
					throw new RuntimeException("无法禁用超级管理员");
				}
			}
		}
		
		SysUserinfo user = this.get(userId);

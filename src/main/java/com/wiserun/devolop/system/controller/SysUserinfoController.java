...
**/
//以下数据可用于配置模块编辑权限
// insertOrUpdateSysUserinfo,insertSysUserinfo.do,updateSysUserinfo.do,delSysUserinfo.do,lgdelSysUserinfo.do,batchSysUserinfo
//以下数据可用于配置查看权限
// listSysUserinfo.do,findRootSysUserinfo.do,findSysUserinfo.do,getSysUserinfo.do
//以下数据一般挺可作为公共角色权限
// findRootSysUserinfo.do,findSysUserinfo.do

@Controller
public class SysUserinfoController extends
		CrudController<SysUserinfo, SysUserinfoService> {

	private final Logger logger = Logger.getLogger(this.getClass());
	
	@Autowired
	private SysUserinfoService vService;
	@Autowired
	private SysUserinfoDao vDao;
	...
	public @ResponseBody SysUserinfo getSysUserinfo(String id) {
 	    return this.getService().get(id);	 //返回对象
	}  


    //列表查询-适用于datagrid 预定义方法(无特殊原因不要修改-除非涉及是否分页，业务逻辑在service中queryBefore方法修改)
    @RequestMapping("/listSysUserinfo.do")
	public @ResponseBody GridResult listSysUserinfo(@RequestBody SysUserinfoSch searchEntity) {
     ...
     //查询（不带权限）-返回LIST 预定义方法  不通过查询类检索 一般用于返回根租户全局配置  (无特殊原因不要修改，业务逻辑在service中queryNoLimitBefore方法修改)
      @RequestMapping("/findNoLimitSysUserinfo.do")
	public @ResponseBody List<SysUserinfo> findNoLimitSysUserinfo() {
    ...
    //查询-返回LIST 预定义方法  不通过查询类检索 一般用于后台查询 返回本账套的数据 (无特殊原因不要修改，业务逻辑在service中queryBefore方法修改)
      @RequestMapping("/findSysUserinfo.do")
	public @ResponseBody List<SysUserinfo> findSysUserinfo() {
       ...
       */
      @RequestMapping("/insertOrUpdateSysUserinfo.do")
    	public @ResponseBody MessageResult insertOrUpdateSysUserinfo(@RequestBody SysUserinfo entity) {
    	  	DefaultMessageResult mes; //消息对象
    	    if (null==entity) return this.error("无数据，放弃保存!");
    	    //预处理密码进行加密
    	    String password = entity.getPassword(); // 前台MD5加密
  	 ...
  	 */
     @RequestMapping("/insertSysUserinfo.do")
  	public @ResponseBody MessageResult insertSysUserinfo(@RequestBody SysUserinfo entity) {
  		DefaultMessageResult mes; //消息对象
	 ...
	 */
     @RequestMapping("/delSysUserinfo.do")
	public @ResponseBody MessageResult delSysUserinfo(@RequestBody SysUserinfo entity) {
		DefaultMessageResult mes; //消息对象
  	 ...
  	 * @return  MessageResult 失败则返回原因
  	 */
     @RequestMapping("/lgdelSysUserinfo.do")
	public @ResponseBody MessageResult lgdelSysUserinfo(@RequestBody SysUserinfo entity) {
  		DefaultMessageResult mes; //消息对象
 	 ...
 	 * @return  MessageResult 失败则返回原因
 	 */
     @RequestMapping("/updateSysUserinfo.do")
 	public @ResponseBody MessageResult updateSysUserinfo(@RequestBody SysUserinfo entity) {
 		DefaultMessageResult mes; //消息对象
      ...
      * @return  MessageResult 失败则返回原因
      */
     @RequestMapping("/batchSysUserinfo.do")
	public @ResponseBody MessageResult batchSysUserinfo(String id) {
    	 	 ...
    	 	 */
    	 	try {
    	 		mes=this.getService().batchByCheckSysUserinfo(map);
				return mes ;
			} catch (Exception e) {
     ...
     private void setGridFooter(ExpressionQuery query,GridResult grid){
         /*//样例--
    	 //页脚处理
    	 List<SysUserinfo> result = new ArrayList<SysUserinfo>();

...
import com.wiserun.devolop.system.service.SysCommontypeService;
...
**/
//以下数据可用于配置模块编辑权限
// insertOrUpdateSysCommontype,insertSysCommontype.do,updateSysCommontype.do,delSysCommontype.do,lgdelSysCommontype.do,batchSysCommontype
//以下数据可用于配置查看权限
// listSysCommontype.do,findRootSysCommontype.do,findSysCommontype.do,getSysCommontype.do
//以下数据一般挺可作为公共角色权限
// findRootSysCommontype.do,findSysCommontype.do

@Controller
public class SysCommontypeController extends
		CrudController<SysCommontype, SysCommontypeService> {

	private final Logger logger = Logger.getLogger(this.getClass());
	
	@Autowired
	private SysCommontypeService vService;
	@Autowired
	private SysCommontypeDao vDao;
    ...
    @RequestMapping("/getSysCommontype.do")
	public @ResponseBody SysCommontype getSysCommontype(String id) {
 	    return this.getService().get(id);	 //返回对象
	}  


    //列表查询-适用于datagrid 预定义方法(无特殊原因不要修改-除非涉及是否分页，业务逻辑在service中queryBefore方法修改)
    @RequestMapping("/listSysCommontype.do")
	public @ResponseBody GridResult listSysCommontype(@RequestBody SysCommontypeSch searchEntity) {
     ...
     //查询（不带权限）-返回LIST 预定义方法  不通过查询类检索 一般用于返回根租户全局配置  (无特殊原因不要修改，业务逻辑在service中queryNoLimitBefore方法修改)
      @RequestMapping("/findNoLimitSysCommontype.do")
	public @ResponseBody List<SysCommontype> findNoLimitSysCommontype() {
    ...
    //查询-返回LIST 预定义方法  不通过查询类检索 一般用于后台查询 返回本账套的数据 (无特殊原因不要修改，业务逻辑在service中queryBefore方法修改)
      @RequestMapping("/findSysCommontype.do")
	public @ResponseBody List<SysCommontype> findSysCommontype() {
       ...
       */
      @RequestMapping("/insertOrUpdateSysCommontype.do")
    	public @ResponseBody MessageResult insertOrUpdateSysCommontype(@RequestBody SysCommontype entity) {
  	 ...
  	 */
     @RequestMapping("/insertSysCommontype.do")
  	public @ResponseBody MessageResult insertSysCommontype(@RequestBody SysCommontype entity) {
  		DefaultMessageResult mes; //消息对象
	 ...
	 */
     @RequestMapping("/delSysCommontype.do")
	public @ResponseBody MessageResult delSysCommontype(@RequestBody SysCommontype entity) {
		DefaultMessageResult mes; //消息对象
  	 ...
  	 * @return  MessageResult 失败则返回原因
  	 */
     @RequestMapping("/lgdelSysCommontype.do")
	public @ResponseBody MessageResult lgdelSysCommontype(@RequestBody SysCommontype entity) {
  		DefaultMessageResult mes; //消息对象
 	 ...
 	 * @return  MessageResult 失败则返回原因
 	 */
     @RequestMapping("/updateSysCommontype.do")
 	public @ResponseBody MessageResult updateSysCommontype(@RequestBody SysCommontype entity) {
 		DefaultMessageResult mes; //消息对象
      ...
      * @return  MessageResult 失败则返回原因
      */
     @RequestMapping("/batchSysCommontype.do")
	public @ResponseBody MessageResult batchSysCommontype(String id) {
    	 	 ...
    	 	 */
    	 	try {
    	 		mes=this.getService().batchByCheckSysCommontype(map);
				return mes ;
			} catch (Exception e) {
     ...
     private void setGridFooter(ExpressionQuery query,GridResult grid){
         /*//样例--
    	 //页脚处理
    	 List<SysCommontype> result = new ArrayList<SysCommontype>();

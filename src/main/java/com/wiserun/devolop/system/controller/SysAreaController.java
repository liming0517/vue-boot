...
import com.wiserun.devolop.system.service.SysAreaService;
...
**/
//以下数据可用于配置模块编辑权限
// insertOrUpdateSysArea,insertSysArea.do,updateSysArea.do,delSysArea.do,lgdelSysArea.do,batchSysArea
//以下数据可用于配置查看权限
// listSysArea.do,findRootSysArea.do,findSysArea.do,getSysArea.do
//以下数据一般挺可作为公共角色权限
// findRootSysArea.do,findSysArea.do

@Controller
public class SysAreaController extends
		CrudController<SysArea, SysAreaService> {

	private final Logger logger = Logger.getLogger(this.getClass());
	
	@Autowired
	private SysAreaService vService;
	@Autowired
	private SysAreaDao vDao;
    ...
    @RequestMapping("/getSysArea.do")
	public @ResponseBody SysArea getSysArea(String id) {
 	    return this.getService().get(id);	 //返回对象
	}  


    //列表查询-适用于datagrid 预定义方法(无特殊原因不要修改-除非涉及是否分页，业务逻辑在service中queryBefore方法修改)
    @RequestMapping("/listSysArea.do")
	public @ResponseBody GridResult listSysArea(@RequestBody SysAreaSch searchEntity) {
     ...
     //查询（不带权限）-返回LIST 预定义方法  不通过查询类检索 一般用于返回根租户全局配置  (无特殊原因不要修改，业务逻辑在service中queryNoLimitBefore方法修改)
      @RequestMapping("/findNoLimitSysArea.do")
	public @ResponseBody List<SysArea> findNoLimitSysArea() {
    ...
    //查询-返回LIST 预定义方法  不通过查询类检索 一般用于后台查询 返回本账套的数据 (无特殊原因不要修改，业务逻辑在service中queryBefore方法修改)
      @RequestMapping("/findSysArea.do")
	public @ResponseBody List<SysArea> findSysArea() {
       ...
       */
      @RequestMapping("/insertOrUpdateSysArea.do")
    	public @ResponseBody MessageResult insertOrUpdateSysArea(@RequestBody SysArea entity) {
  	 ...
  	 */
     @RequestMapping("/insertSysArea.do")
  	public @ResponseBody MessageResult insertSysArea(@RequestBody SysArea entity) {
  		DefaultMessageResult mes; //消息对象
	 ...
	 */
     @RequestMapping("/delSysArea.do")
	public @ResponseBody MessageResult delSysArea(@RequestBody SysArea entity) {
		DefaultMessageResult mes; //消息对象
  	 ...
  	 * @return  MessageResult 失败则返回原因
  	 */
     @RequestMapping("/lgdelSysArea.do")
	public @ResponseBody MessageResult lgdelSysArea(@RequestBody SysArea entity) {
  		DefaultMessageResult mes; //消息对象
 	 ...
 	 * @return  MessageResult 失败则返回原因
 	 */
     @RequestMapping("/updateSysArea.do")
 	public @ResponseBody MessageResult updateSysArea(@RequestBody SysArea entity) {
 		DefaultMessageResult mes; //消息对象
      ...
      * @return  MessageResult 失败则返回原因
      */
     @RequestMapping("/batchSysArea.do")
	public @ResponseBody MessageResult batchSysArea(String id) {
    	 	 ...
    	 	 */
    	 	try {
    	 		mes=this.getService().batchByCheckSysArea(map);
				return mes ;
			} catch (Exception e) {

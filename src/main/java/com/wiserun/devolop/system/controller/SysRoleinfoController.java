...
import com.wiserun.devolop.system.service.SysRoleinfoService;
...
**/
//以下数据可用于配置模块编辑权限
// insertOrUpdateSysRoleinfo,insertSysRoleinfo.do,updateSysRoleinfo.do,delSysRoleinfo.do,lgdelSysRoleinfo.do,batchSysRoleinfo
//以下数据可用于配置查看权限
// listSysRoleinfo.do,findRootSysRoleinfo.do,findSysRoleinfo.do,getSysRoleinfo.do
//以下数据一般挺可作为公共角色权限
// findRootSysRoleinfo.do,findSysRoleinfo.do

@Controller
public class SysRoleinfoController extends
		CrudController<SysRoleinfo, SysRoleinfoService> {

	private final Logger logger = Logger.getLogger(this.getClass());
	
	@Autowired
	private SysRoleinfoService vService;
	@Autowired
	private SysRoleinfoDao vDao;
    ...
    @RequestMapping("/getSysRoleinfo.do")
	public @ResponseBody SysRoleinfo getSysRoleinfo(String id) {
 	    return this.getService().get(id);	 //返回对象
	}  


    //列表查询-适用于datagrid 预定义方法(无特殊原因不要修改-除非涉及是否分页，业务逻辑在service中queryBefore方法修改)
    @RequestMapping("/listSysRoleinfo.do")
	public @ResponseBody GridResult listSysRoleinfo(@RequestBody SysRoleinfoSch searchEntity) {
     ...
     //查询（不带权限）-返回LIST 预定义方法  不通过查询类检索 一般用于返回根租户全局配置  (无特殊原因不要修改，业务逻辑在service中queryNoLimitBefore方法修改)
      @RequestMapping("/findNoLimitSysRoleinfo.do")
	public @ResponseBody List<SysRoleinfo> findNoLimitSysRoleinfo() {
    ...
    //查询-返回LIST 预定义方法  不通过查询类检索 一般用于后台查询 返回本账套的数据 (无特殊原因不要修改，业务逻辑在service中queryBefore方法修改)
      @RequestMapping("/findSysRoleinfo.do")
	public @ResponseBody List<SysRoleinfo> findSysRoleinfo() {
       ...
       */
      @RequestMapping("/insertOrUpdateSysRoleinfo.do")
    	public @ResponseBody MessageResult insertOrUpdateSysRoleinfo(@RequestBody SysRoleinfo entity) {
  	 ...
  	 */
     @RequestMapping("/insertSysRoleinfo.do")
  	public @ResponseBody MessageResult insertSysRoleinfo(@RequestBody SysRoleinfo entity) {
  		DefaultMessageResult mes; //消息对象
	 ...
	 */
     @RequestMapping("/delSysRoleinfo.do")
	public @ResponseBody MessageResult delSysRoleinfo(@RequestBody SysRoleinfo entity) {
		DefaultMessageResult mes; //消息对象
  	 ...
  	 * @return  MessageResult 失败则返回原因
  	 */
     @RequestMapping("/lgdelSysRoleinfo.do")
	public @ResponseBody MessageResult lgdelSysRoleinfo(@RequestBody SysRoleinfo entity) {
  		DefaultMessageResult mes; //消息对象
 	 ...
 	 * @return  MessageResult 失败则返回原因
 	 */
     @RequestMapping("/updateSysRoleinfo.do")
 	public @ResponseBody MessageResult updateSysRoleinfo(@RequestBody SysRoleinfo entity) {
 		DefaultMessageResult mes; //消息对象
      ...
      * @return  MessageResult 失败则返回原因
      */
     @RequestMapping("/batchSysRoleinfo.do")
	public @ResponseBody MessageResult batchSysRoleinfo(String id) {
    	 	 ...
    	 	 */
    	 	try {
    	 		mes=this.getService().batchByCheckSysRoleinfo(map);
				return mes ;
			} catch (Exception e) {
     ...
     private void setGridFooter(ExpressionQuery query,GridResult grid){
         /*//样例--
    	 //页脚处理
    	 List<SysRoleinfo> result = new ArrayList<SysRoleinfo>();

...
**/
//以下数据可用于配置模块编辑权限
// insertOrUpdateSysFunction,insertSysFunction.do,updateSysFunction.do,delSysFunction.do,lgdelSysFunction.do,batchSysFunction
//以下数据可用于配置查看权限
// listSysFunction.do,findRootSysFunction.do,findSysFunction.do,getSysFunction.do
//以下数据一般挺可作为公共角色权限
// findRootSysFunction.do,findSysFunction.do

@Controller
public class SysFunctionController extends
		CrudController<SysFunction, SysFunctionService> {

	private final Logger logger = Logger.getLogger(this.getClass());
	
	@Autowired
	private SysFunctionService vService;
	@Autowired
	private SysFunctionDao vDao;
    ...
    @RequestMapping("/getSysFunction.do")
	public @ResponseBody SysFunction getSysFunction(String id) {
 	    return this.getService().get(id);	 //返回对象
	}  


    //列表查询-适用于datagrid 预定义方法(无特殊原因不要修改-除非涉及是否分页，业务逻辑在service中queryBefore方法修改)
    @RequestMapping("/listSysFunction.do")
	public @ResponseBody GridResult listSysFunction(@RequestBody SysFunctionSch searchEntity) {
     ...
     //查询（不带权限）-返回LIST 预定义方法  不通过查询类检索 一般用于返回根租户全局配置  (无特殊原因不要修改，业务逻辑在service中queryNoLimitBefore方法修改)
      @RequestMapping("/findNoLimitSysFunction.do")
	public @ResponseBody List<SysFunction> findNoLimitSysFunction() {
    ...
    //查询-返回LIST 预定义方法  不通过查询类检索 一般用于后台查询 返回本账套的数据 (无特殊原因不要修改，业务逻辑在service中queryBefore方法修改)
      @RequestMapping("/findSysFunction.do")
	public @ResponseBody List<SysFunction> findSysFunction() {
       ...
       */
      @RequestMapping("/insertOrUpdateSysFunction.do")
    	public @ResponseBody MessageResult insertOrUpdateSysFunction(@RequestBody SysFunction entity) {
  	 ...
  	 */
     @RequestMapping("/insertSysFunction.do")
  	public @ResponseBody MessageResult insertSysFunction(@RequestBody SysFunction entity) {
  		DefaultMessageResult mes; //消息对象
	 ...
	 */
     @RequestMapping("/delSysFunction.do")
	public @ResponseBody MessageResult delSysFunction(@RequestBody SysFunction entity) {
		DefaultMessageResult mes; //消息对象
  	 ...
  	 * @return  MessageResult 失败则返回原因
  	 */
     @RequestMapping("/lgdelSysFunction.do")
	public @ResponseBody MessageResult lgdelSysFunction(@RequestBody SysFunction entity) {
  		DefaultMessageResult mes; //消息对象
 	 ...
 	 * @return  MessageResult 失败则返回原因
 	 */
     @RequestMapping("/updateSysFunction.do")
 	public @ResponseBody MessageResult updateSysFunction(@RequestBody SysFunction entity) {
 		DefaultMessageResult mes; //消息对象
      ...
      * @return  MessageResult 失败则返回原因
      */
     @RequestMapping("/batchSysFunction.do")
	public @ResponseBody MessageResult batchSysFunction(String id) {
    	 	 ...
    	 	 */
    	 	try {
    	 		mes=this.getService().batchByCheckSysFunction(map);
				return mes ;
			} catch (Exception e) {

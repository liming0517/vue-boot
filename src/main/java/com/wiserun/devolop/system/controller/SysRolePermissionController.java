...
import com.wiserun.devolop.system.service.SysRolePermissionService;
...
**/
//以下数据可用于配置模块编辑权限
// insertOrUpdateSysRolePermission,insertSysRolePermission.do,updateSysRolePermission.do,delSysRolePermission.do,lgdelSysRolePermission.do,batchSysRolePermission
//以下数据可用于配置查看权限
// listSysRolePermission.do,findRootSysRolePermission.do,findSysRolePermission.do,getSysRolePermission.do
//以下数据一般挺可作为公共角色权限
// findRootSysRolePermission.do,findSysRolePermission.do

@Controller
public class SysRolePermissionController extends
		CrudController<SysRolePermission, SysRolePermissionService> {

	private final Logger logger = Logger.getLogger(this.getClass());
	
	@Autowired
	private SysRolePermissionService vService;
	@Autowired
	private SysRolePermissionDao vDao;
    ...
    @RequestMapping("/getSysRolePermission.do")
	public @ResponseBody SysRolePermission getSysRolePermission(String id) {
 	    return this.getService().get(id);	 //返回对象
	}  


    //列表查询-适用于datagrid 预定义方法(无特殊原因不要修改-除非涉及是否分页，业务逻辑在service中queryBefore方法修改)
    @RequestMapping("/listSysRolePermission.do")
	public @ResponseBody GridResult listSysRolePermission(@RequestBody SysRolePermissionSch searchEntity) {
     ...
     //查询（不带权限）-返回LIST 预定义方法  不通过查询类检索 一般用于返回根租户全局配置  (无特殊原因不要修改，业务逻辑在service中queryNoLimitBefore方法修改)
      @RequestMapping("/findNoLimitSysRolePermission.do")
	public @ResponseBody List<SysRolePermission> findNoLimitSysRolePermission() {
    ...
    //查询-返回LIST 预定义方法  不通过查询类检索 一般用于后台查询 返回本账套的数据 (无特殊原因不要修改，业务逻辑在service中queryBefore方法修改)
      @RequestMapping("/findSysRolePermission.do")
	public @ResponseBody List<SysRolePermission> findSysRolePermission() {
       ...
       */
      @RequestMapping("/insertOrUpdateSysRolePermission.do")
    	public @ResponseBody MessageResult insertOrUpdateSysRolePermission(@RequestBody SysRolePermission entity) {
  	 ...
  	 */
     @RequestMapping("/insertSysRolePermission.do")
  	public @ResponseBody MessageResult insertSysRolePermission(@RequestBody SysRolePermission entity) {
  		DefaultMessageResult mes; //消息对象
	 ...
	 */
     @RequestMapping("/delSysRolePermission.do")
	public @ResponseBody MessageResult delSysRolePermission(@RequestBody SysRolePermission entity) {
		DefaultMessageResult mes; //消息对象
  	 ...
  	 * @return  MessageResult 失败则返回原因
  	 */
     @RequestMapping("/lgdelSysRolePermission.do")
	public @ResponseBody MessageResult lgdelSysRolePermission(@RequestBody SysRolePermission entity) {
  		DefaultMessageResult mes; //消息对象
 	 ...
 	 * @return  MessageResult 失败则返回原因
 	 */
     @RequestMapping("/updateSysRolePermission.do")
 	public @ResponseBody MessageResult updateSysRolePermission(@RequestBody SysRolePermission entity) {
 		DefaultMessageResult mes; //消息对象
      ...
      * @return  MessageResult 失败则返回原因
      */
     @RequestMapping("/batchSysRolePermission.do")
	public @ResponseBody MessageResult batchSysRolePermission(String id) {
    	 	 ...
    	 	 */
    	 	try {
    	 		mes=this.getService().batchByCheckSysRolePermission(map);
				return mes ;
			} catch (Exception e) {
     ...
     private void setGridFooter(ExpressionQuery query,GridResult grid){
         /*//样例--
    	 //页脚处理
    	 List<SysRolePermission> result = new ArrayList<SysRolePermission>();

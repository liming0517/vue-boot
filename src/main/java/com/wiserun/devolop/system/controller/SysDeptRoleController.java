...
import com.wiserun.devolop.system.service.SysDeptRoleService;
...
**/
//以下数据可用于配置模块编辑权限
// insertOrUpdateSysDeptRole,insertSysDeptRole.do,updateSysDeptRole.do,delSysDeptRole.do,lgdelSysDeptRole.do,batchSysDeptRole
//以下数据可用于配置查看权限
// listSysDeptRole.do,findRootSysDeptRole.do,findSysDeptRole.do,getSysDeptRole.do
//以下数据一般挺可作为公共角色权限
// findRootSysDeptRole.do,findSysDeptRole.do

@Controller
public class SysDeptRoleController extends
		CrudController<SysDeptRole, SysDeptRoleService> {

	private final Logger logger = Logger.getLogger(this.getClass());
	
	@Autowired
	private SysDeptRoleService vService;
	@Autowired
	private SysDeptRoleDao vDao;
    ...
    @RequestMapping("/getSysDeptRole.do")
	public @ResponseBody SysDeptRole getSysDeptRole(String id) {
 	    return this.getService().get(id);	 //返回对象
	}  


    //列表查询-适用于datagrid 预定义方法(无特殊原因不要修改-除非涉及是否分页，业务逻辑在service中queryBefore方法修改)
    @RequestMapping("/listSysDeptRole.do")
	public @ResponseBody GridResult listSysDeptRole(@RequestBody SysDeptRoleSch searchEntity) {
     ...
     //查询（不带权限）-返回LIST 预定义方法  不通过查询类检索 一般用于返回根租户全局配置  (无特殊原因不要修改，业务逻辑在service中queryNoLimitBefore方法修改)
      @RequestMapping("/findNoLimitSysDeptRole.do")
	public @ResponseBody List<SysDeptRole> findNoLimitSysDeptRole() {
    ...
    //查询-返回LIST 预定义方法  不通过查询类检索 一般用于后台查询 返回本账套的数据 (无特殊原因不要修改，业务逻辑在service中queryBefore方法修改)
      @RequestMapping("/findSysDeptRole.do")
	public @ResponseBody List<SysDeptRole> findSysDeptRole() {
       ...
       */
      @RequestMapping("/insertOrUpdateSysDeptRole.do")
    	public @ResponseBody MessageResult insertOrUpdateSysDeptRole(@RequestBody SysDeptRole entity) {
  	 ...
  	 */
     @RequestMapping("/insertSysDeptRole.do")
  	public @ResponseBody MessageResult insertSysDeptRole(@RequestBody SysDeptRole entity) {
  		DefaultMessageResult mes; //消息对象
	 ...
	 */
     @RequestMapping("/delSysDeptRole.do")
	public @ResponseBody MessageResult delSysDeptRole(@RequestBody SysDeptRole entity) {
		DefaultMessageResult mes; //消息对象
  	 ...
  	 * @return  MessageResult 失败则返回原因
  	 */
     @RequestMapping("/lgdelSysDeptRole.do")
	public @ResponseBody MessageResult lgdelSysDeptRole(@RequestBody SysDeptRole entity) {
  		DefaultMessageResult mes; //消息对象
 	 ...
 	 * @return  MessageResult 失败则返回原因
 	 */
     @RequestMapping("/updateSysDeptRole.do")
 	public @ResponseBody MessageResult updateSysDeptRole(@RequestBody SysDeptRole entity) {
 		DefaultMessageResult mes; //消息对象
      ...
      * @return  MessageResult 失败则返回原因
      */
     @RequestMapping("/batchSysDeptRole.do")
	public @ResponseBody MessageResult batchSysDeptRole(String id) {
    	 	 ...
    	 	 */
    	 	try {
    	 		mes=this.getService().batchByCheckSysDeptRole(map);
				return mes ;
			} catch (Exception e) {
     ...
     private void setGridFooter(ExpressionQuery query,GridResult grid){
         /*//样例--
    	 //页脚处理
    	 List<SysDeptRole> result = new ArrayList<SysDeptRole>();

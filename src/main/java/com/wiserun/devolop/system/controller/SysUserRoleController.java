...
**/
//以下数据可用于配置模块编辑权限
// insertOrUpdateSysUserRole,insertSysUserRole.do,updateSysUserRole.do,delSysUserRole.do,lgdelSysUserRole.do,batchSysUserRole
//以下数据可用于配置查看权限
// listSysUserRole.do,findRootSysUserRole.do,findSysUserRole.do,getSysUserRole.do
//以下数据一般挺可作为公共角色权限
// findRootSysUserRole.do,findSysUserRole.do

@Controller
public class SysUserRoleController extends
		CrudController<SysUserRole, SysUserRoleService> {

	private final Logger logger = Logger.getLogger(this.getClass());
	
	@Autowired
	private SysUserRoleService vService;
	@Autowired
	private SysUserRoleDao vDao;
    ...
    @RequestMapping("/getSysUserRole.do")
	public @ResponseBody SysUserRole getSysUserRole(String id) {
 	    return this.getService().get(id);	 //返回对象
	}  


    //列表查询-适用于datagrid 预定义方法(无特殊原因不要修改-除非涉及是否分页，业务逻辑在service中queryBefore方法修改)
    @RequestMapping("/listSysUserRole.do")
	public @ResponseBody GridResult listSysUserRole(@RequestBody SysUserRoleSch searchEntity) {
     ...
     //查询（不带权限）-返回LIST 预定义方法  不通过查询类检索 一般用于返回根租户全局配置  (无特殊原因不要修改，业务逻辑在service中queryNoLimitBefore方法修改)
      @RequestMapping("/findNoLimitSysUserRole.do")
	public @ResponseBody List<SysUserRole> findNoLimitSysUserRole() {
    ...
    //查询-返回LIST 预定义方法  不通过查询类检索 一般用于后台查询 返回本账套的数据 (无特殊原因不要修改，业务逻辑在service中queryBefore方法修改)
      @RequestMapping("/findSysUserRole.do")
	public @ResponseBody List<SysUserRole> findSysUserRole() {
       ...
       */
      @RequestMapping("/insertOrUpdateSysUserRole.do")
    	public @ResponseBody MessageResult insertOrUpdateSysUserRole(@RequestBody SysUserRole entity) {
  	 ...
  	 */
     @RequestMapping("/insertSysUserRole.do")
  	public @ResponseBody MessageResult insertSysUserRole(@RequestBody SysUserRole entity) {
  		DefaultMessageResult mes; //消息对象
	 ...
	 */
     @RequestMapping("/delSysUserRole.do")
	public @ResponseBody MessageResult delSysUserRole(@RequestBody SysUserRole entity) {
		DefaultMessageResult mes; //消息对象
  	 ...
  	 * @return  MessageResult 失败则返回原因
  	 */
     @RequestMapping("/lgdelSysUserRole.do")
	public @ResponseBody MessageResult lgdelSysUserRole(@RequestBody SysUserRole entity) {
  		DefaultMessageResult mes; //消息对象
 	 ...
 	 * @return  MessageResult 失败则返回原因
 	 */
     @RequestMapping("/updateSysUserRole.do")
 	public @ResponseBody MessageResult updateSysUserRole(@RequestBody SysUserRole entity) {
 		DefaultMessageResult mes; //消息对象
      ...
      * @return  MessageResult 失败则返回原因
      */
     @RequestMapping("/batchSysUserRole.do")
	public @ResponseBody MessageResult batchSysUserRole(String id) {
    	 	 ...
    	 	 */
    	 	try {
    	 		mes=this.getService().batchByCheckSysUserRole(map);
				return mes ;
			} catch (Exception e) {

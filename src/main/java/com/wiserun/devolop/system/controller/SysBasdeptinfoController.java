...
import com.wiserun.devolop.system.service.SysBasdeptinfoService;
...
**/
//以下数据可用于配置模块编辑权限
// insertOrUpdateSysBasdeptinfo,insertSysBasdeptinfo.do,updateSysBasdeptinfo.do,delSysBasdeptinfo.do,lgdelSysBasdeptinfo.do,batchSysBasdeptinfo
//以下数据可用于配置查看权限
// listSysBasdeptinfo.do,findRootSysBasdeptinfo.do,findSysBasdeptinfo.do,getSysBasdeptinfo.do
//以下数据一般挺可作为公共角色权限
// findRootSysBasdeptinfo.do,findSysBasdeptinfo.do

@Controller
public class SysBasdeptinfoController extends
		CrudController<SysBasdeptinfo, SysBasdeptinfoService> {

	private final Logger logger = Logger.getLogger(this.getClass());
	
	@Autowired
	private SysBasdeptinfoService vService;
	@Autowired
	private SysBasdeptinfoDao vDao;
    ...
    @RequestMapping("/getSysBasdeptinfo.do")
	public @ResponseBody SysBasdeptinfo getSysBasdeptinfo(String id) {
 	    return this.getService().get(id);	 //返回对象
	}  


    //列表查询-适用于datagrid 预定义方法(无特殊原因不要修改-除非涉及是否分页，业务逻辑在service中queryBefore方法修改)
    @RequestMapping("/listSysBasdeptinfo.do")
	public @ResponseBody GridResult listSysBasdeptinfo(@RequestBody SysBasdeptinfoSch searchEntity) {
     ...
     //查询（不带权限）-返回LIST 预定义方法  不通过查询类检索 一般用于返回根租户全局配置  (无特殊原因不要修改，业务逻辑在service中queryNoLimitBefore方法修改)
      @RequestMapping("/findNoLimitSysBasdeptinfo.do")
	public @ResponseBody List<SysBasdeptinfo> findNoLimitSysBasdeptinfo() {
    ...
    //查询-返回LIST 预定义方法  不通过查询类检索 一般用于后台查询 返回本账套的数据 (无特殊原因不要修改，业务逻辑在service中queryBefore方法修改)
      @RequestMapping("/findSysBasdeptinfo.do")
	public @ResponseBody List<SysBasdeptinfo> findSysBasdeptinfo() {
       ...
       */
      @RequestMapping("/insertOrUpdateSysBasdeptinfo.do")
    	public @ResponseBody MessageResult insertOrUpdateSysBasdeptinfo(@RequestBody SysBasdeptinfo entity) {
  	 ...
  	 */
     @RequestMapping("/insertSysBasdeptinfo.do")
  	public @ResponseBody MessageResult insertSysBasdeptinfo(@RequestBody SysBasdeptinfo entity) {
  		DefaultMessageResult mes; //消息对象
	 ...
	 */
     @RequestMapping("/delSysBasdeptinfo.do")
	public @ResponseBody MessageResult delSysBasdeptinfo(@RequestBody SysBasdeptinfo entity) {
		DefaultMessageResult mes; //消息对象
  	 ...
  	 * @return  MessageResult 失败则返回原因
  	 */
     @RequestMapping("/lgdelSysBasdeptinfo.do")
	public @ResponseBody MessageResult lgdelSysBasdeptinfo(@RequestBody SysBasdeptinfo entity) {
  		DefaultMessageResult mes; //消息对象
 	 ...
 	 * @return  MessageResult 失败则返回原因
 	 */
     @RequestMapping("/updateSysBasdeptinfo.do")
 	public @ResponseBody MessageResult updateSysBasdeptinfo(@RequestBody SysBasdeptinfo entity) {
 		DefaultMessageResult mes; //消息对象
      ...
      * @return  MessageResult 失败则返回原因
      */
     @RequestMapping("/batchSysBasdeptinfo.do")
	public @ResponseBody MessageResult batchSysBasdeptinfo(String id) {
    	 	 ...
    	 	 */
    	 	try {
    	 		mes=this.getService().batchByCheckSysBasdeptinfo(map);
				return mes ;
			} catch (Exception e) {
     ...
     private void setGridFooter(ExpressionQuery query,GridResult grid){
         /*//样例--
    	 //页脚处理
    	 List<SysBasdeptinfo> result = new ArrayList<SysBasdeptinfo>();

...
**/
//以下数据可用于配置模块编辑权限
// insertOrUpdateSysResTab,insertSysResTab.do,updateSysResTab.do,delSysResTab.do,lgdelSysResTab.do,batchSysResTab
//以下数据可用于配置查看权限
// listSysResTab.do,findRootSysResTab.do,findSysResTab.do,getSysResTab.do
//以下数据一般挺可作为公共角色权限
// findRootSysResTab.do,findSysResTab.do

@Controller
public class SysResTabController extends
		CrudController<SysResTab, SysResTabService> {

	private final Logger logger = Logger.getLogger(this.getClass());
	
	@Autowired
	private SysResTabService vService;
	@Autowired
	private SysResTabDao vDao;
    ...
    @RequestMapping("/getSysResTab.do")
	public @ResponseBody SysResTab getSysResTab(String id) {
 	    return this.getService().get(id);	 //返回对象
	}  


    //列表查询-适用于datagrid 预定义方法(无特殊原因不要修改-除非涉及是否分页，业务逻辑在service中queryBefore方法修改)
    @RequestMapping("/listSysResTab.do")
	public @ResponseBody GridResult listSysResTab(@RequestBody SysResTabSch searchEntity) {
     ...
     //查询（不带权限）-返回LIST 预定义方法  不通过查询类检索 一般用于返回根租户全局配置  (无特殊原因不要修改，业务逻辑在service中queryNoLimitBefore方法修改)
      @RequestMapping("/findNoLimitSysResTab.do")
	public @ResponseBody List<SysResTab> findNoLimitSysResTab() {
    ...
    //查询-返回LIST 预定义方法  不通过查询类检索 一般用于后台查询 返回本账套的数据 (无特殊原因不要修改，业务逻辑在service中queryBefore方法修改)
      @RequestMapping("/findSysResTab.do")
	public @ResponseBody List<SysResTab> findSysResTab() {
       ...
       */
      @RequestMapping("/insertOrUpdateSysResTab.do")
    	public @ResponseBody MessageResult insertOrUpdateSysResTab(@RequestBody SysResTab entity) {
  	 ...
  	 */
     @RequestMapping("/insertSysResTab.do")
  	public @ResponseBody MessageResult insertSysResTab(@RequestBody SysResTab entity) {
  		DefaultMessageResult mes; //消息对象
	 ...
	 */
     @RequestMapping("/delSysResTab.do")
	public @ResponseBody MessageResult delSysResTab(@RequestBody SysResTab entity) {
		DefaultMessageResult mes; //消息对象
  	 ...
  	 * @return  MessageResult 失败则返回原因
  	 */
     @RequestMapping("/lgdelSysResTab.do")
	public @ResponseBody MessageResult lgdelSysResTab(@RequestBody SysResTab entity) {
  		DefaultMessageResult mes; //消息对象
 	 ...
 	 * @return  MessageResult 失败则返回原因
 	 */
     @RequestMapping("/updateSysResTab.do")
 	public @ResponseBody MessageResult updateSysResTab(@RequestBody SysResTab entity) {
 		DefaultMessageResult mes; //消息对象
      ...
      * @return  MessageResult 失败则返回原因
      */
     @RequestMapping("/batchSysResTab.do")
	public @ResponseBody MessageResult batchSysResTab(String id) {
    	 	 ...
    	 	 */
    	 	try {
    	 		mes=this.getService().batchByCheckSysResTab(map);
				return mes ;
			} catch (Exception e) {
     ...
     private void setGridFooter(ExpressionQuery query,GridResult grid){
         /*//样例--
    	 //页脚处理
    	 List<SysResTab> result = new ArrayList<SysResTab>();

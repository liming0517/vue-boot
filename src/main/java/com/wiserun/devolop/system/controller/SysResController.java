...
**/
//以下数据可用于配置模块编辑权限
// insertOrUpdateSysRes,insertSysRes.do,updateSysRes.do,delSysRes.do,lgdelSysRes.do,batchSysRes
//以下数据可用于配置查看权限
// listSysRes.do,findRootSysRes.do,findSysRes.do,getSysRes.do
//以下数据一般挺可作为公共角色权限
// findRootSysRes.do,findSysRes.do

@Controller
public class SysResController extends
		CrudController<SysRes, SysResService> {

	private final Logger logger = Logger.getLogger(this.getClass());
	
	@Autowired
	private SysResService vService;
	@Autowired
	private SysResDao vDao;
    ...
    @RequestMapping("/getSysRes.do")
	public @ResponseBody SysRes getSysRes(String id) {
 	    return this.getService().get(id);	 //返回对象
	}  


    //列表查询-适用于datagrid 预定义方法(无特殊原因不要修改-除非涉及是否分页，业务逻辑在service中queryBefore方法修改)
    @RequestMapping("/listSysRes.do")
	public @ResponseBody GridResult listSysRes(@RequestBody SysResSch searchEntity) {
     ...
     //查询（不带权限）-返回LIST 预定义方法  不通过查询类检索 一般用于返回根租户全局配置  (无特殊原因不要修改，业务逻辑在service中queryNoLimitBefore方法修改)
      @RequestMapping("/findNoLimitSysRes.do")
	public @ResponseBody List<SysRes> findNoLimitSysRes() {
    ...
    //查询-返回LIST 预定义方法  不通过查询类检索 一般用于后台查询 返回本账套的数据 (无特殊原因不要修改，业务逻辑在service中queryBefore方法修改)
      @RequestMapping("/findSysRes.do")
	public @ResponseBody List<SysRes> findSysRes() {
       ...
       */
      @RequestMapping("/insertOrUpdateSysRes.do")
    	public @ResponseBody MessageResult insertOrUpdateSysRes(@RequestBody SysRes entity) {
  	 ...
  	 */
     @RequestMapping("/insertSysRes.do")
  	public @ResponseBody MessageResult insertSysRes(@RequestBody SysRes entity) {
  		DefaultMessageResult mes; //消息对象
	 ...
	 */
     @RequestMapping("/delSysRes.do")
	public @ResponseBody MessageResult delSysRes(@RequestBody SysRes entity) {
		DefaultMessageResult mes; //消息对象
  	 ...
  	 * @return  MessageResult 失败则返回原因
  	 */
     @RequestMapping("/lgdelSysRes.do")
	public @ResponseBody MessageResult lgdelSysRes(@RequestBody SysRes entity) {
  		DefaultMessageResult mes; //消息对象
 	 ...
 	 * @return  MessageResult 失败则返回原因
 	 */
     @RequestMapping("/updateSysRes.do")
 	public @ResponseBody MessageResult updateSysRes(@RequestBody SysRes entity) {
 		DefaultMessageResult mes; //消息对象
      ...
      * @return  MessageResult 失败则返回原因
      */
     @RequestMapping("/batchSysRes.do")
	public @ResponseBody MessageResult batchSysRes(String id) {
    	 	 ...
    	 	 */
    	 	try {
    	 		mes=this.getService().batchByCheckSysRes(map);
				return mes ;
			} catch (Exception e) {
     ...
     private void setGridFooter(ExpressionQuery query,GridResult grid){
         /*//样例--
    	 //页脚处理
    	 List<SysRes> result = new ArrayList<SysRes>();

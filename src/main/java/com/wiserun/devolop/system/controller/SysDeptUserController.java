...
import com.wiserun.devolop.system.service.SysDeptUserService;
...
**/
//以下数据可用于配置模块编辑权限
// insertOrUpdateSysDeptUser,insertSysDeptUser.do,updateSysDeptUser.do,delSysDeptUser.do,lgdelSysDeptUser.do,batchSysDeptUser
//以下数据可用于配置查看权限
// listSysDeptUser.do,findRootSysDeptUser.do,findSysDeptUser.do,getSysDeptUser.do
//以下数据一般挺可作为公共角色权限
// findRootSysDeptUser.do,findSysDeptUser.do

@Controller
public class SysDeptUserController extends
		CrudController<SysDeptUser, SysDeptUserService> {

	private final Logger logger = Logger.getLogger(this.getClass());
	
	@Autowired
	private SysDeptUserService vService;
	@Autowired
	private SysDeptUserDao vDao;
    ...
    @RequestMapping("/getSysDeptUser.do")
	public @ResponseBody SysDeptUser getSysDeptUser(String id) {
 	    return this.getService().get(id);	 //返回对象
	}  


    //列表查询-适用于datagrid 预定义方法(无特殊原因不要修改-除非涉及是否分页，业务逻辑在service中queryBefore方法修改)
    @RequestMapping("/listSysDeptUser.do")
	public @ResponseBody GridResult listSysDeptUser(@RequestBody SysDeptUserSch searchEntity) {
     ...
     //查询（不带权限）-返回LIST 预定义方法  不通过查询类检索 一般用于返回根租户全局配置  (无特殊原因不要修改，业务逻辑在service中queryNoLimitBefore方法修改)
      @RequestMapping("/findNoLimitSysDeptUser.do")
	public @ResponseBody List<SysDeptUser> findNoLimitSysDeptUser() {
    ...
    //查询-返回LIST 预定义方法  不通过查询类检索 一般用于后台查询 返回本账套的数据 (无特殊原因不要修改，业务逻辑在service中queryBefore方法修改)
      @RequestMapping("/findSysDeptUser.do")
	public @ResponseBody List<SysDeptUser> findSysDeptUser() {
       ...
       */
      @RequestMapping("/insertOrUpdateSysDeptUser.do")
    	public @ResponseBody MessageResult insertOrUpdateSysDeptUser(@RequestBody SysDeptUser entity) {
  	 ...
  	 */
     @RequestMapping("/insertSysDeptUser.do")
  	public @ResponseBody MessageResult insertSysDeptUser(@RequestBody SysDeptUser entity) {
  		DefaultMessageResult mes; //消息对象
	 ...
	 */
     @RequestMapping("/delSysDeptUser.do")
	public @ResponseBody MessageResult delSysDeptUser(@RequestBody SysDeptUser entity) {
		DefaultMessageResult mes; //消息对象
  	 ...
  	 * @return  MessageResult 失败则返回原因
  	 */
     @RequestMapping("/lgdelSysDeptUser.do")
	public @ResponseBody MessageResult lgdelSysDeptUser(@RequestBody SysDeptUser entity) {
  		DefaultMessageResult mes; //消息对象
 	 ...
 	 * @return  MessageResult 失败则返回原因
 	 */
     @RequestMapping("/updateSysDeptUser.do")
 	public @ResponseBody MessageResult updateSysDeptUser(@RequestBody SysDeptUser entity) {
 		DefaultMessageResult mes; //消息对象
      ...
      * @return  MessageResult 失败则返回原因
      */
     @RequestMapping("/batchSysDeptUser.do")
	public @ResponseBody MessageResult batchSysDeptUser(String id) {
    	 	 ...
    	 	 */
    	 	try {
    	 		mes=this.getService().batchByCheckSysDeptUser(map);
				return mes ;
			} catch (Exception e) {
     ...
     private void setGridFooter(ExpressionQuery query,GridResult grid){
         /*//样例--
    	 //页脚处理
    	 List<SysDeptUser> result = new ArrayList<SysDeptUser>();

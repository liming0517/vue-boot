<template>
    <el-table :data="tableData" v-loading.body="tableLoading" element-loading-text="拼命加载中" border fit highlight-current-row row-key="id">
        ...
        <el-table-column label="操作" align="center" width="500px">
            <template slot-scope="scope">
                <el-button v-if="scope.row.level" size="small" @click="emitRoleManage(scope.row.id, 'menu')" type="primary" icon="setting">菜单</el-button>
                <el-button v-if="scope.row.level" size="small" @click="emitRoleManage(scope.row.id, 'resource')" type="default" icon="setting">资源</el-button>
                <el-button size="small" @click="emitRoleManage(scope.row.id, 'user')" type="warning" icon="setting">用户</el-button>
                <el-button v-if="scope.row.level" size="small" @click="emitRoleUpdate(scope.row)" type="gray" icon="edit">修改</el-button>

<template>
    <div class="components-container">
        <div class="filter-container">
            ...
            <el-button @click="showDialog('create')" type="gray"><i class="el-icon-plus"></i> 新增用户</el-button>
        </div>
        ...
        <el-table :data="tableData" v-loading.body="tableLoading" element-loading-text="拼命加载中" stripe border fit highlight-current-row style="width: 100%">
            <el-table-column label="ID" prop="id" align="center" width="70px"></el-table-column>
            ...
            <el-table-column label="部门" align="center">
                <template slot-scope="scope">
                    <span v-for="(dept, index) in scope.row.user_departmentlist" :key="index">
                        <el-tag type="gray"> {{ dept }} </el-tag>
                    </span>
                </template>
            </el-table-column>
        ...
        <el-dialog :title="formTitle" :visible.sync="formVisible">
            <el-form class="small-space" :model="createdItem" :rules="createdRules" label-position="left" label-width="70px" style='width: 60%; margin-left:0px;'>
                <el-form-item label="用户名">
                    <el-input v-model="createdItem.name" placeholder="请填写用户名" value="2ewqdwq"></el-input>
                </el-form-item>
                ...
                <el-form-item label="用户部门">
                    <el-select v-model="createdItem.user_departmentlist" multiple placeholder="请选择用户部门" style="width: 100%">
                        <el-option v-for="item in deptList" :key="item.deptCode" :label="item.cname" :value="item.cname"></el-option>
                    </el-select>
                </el-form-item>
...
export default {
    ...
    methods: {
        ...
        getDeptList(){
            this.$api.system.department.list({
                data:this.deptQuery
            }).then(response => {
        ...
        clearDialog() {
            ...
            this.createdItem.user_departmentlist = [];
        },
        showDialog(type, row = null) {
            ...
            if (type == 'create') {
                ...
                this.createdItem.user_departmentlist = Object.values(row.user_departmentlist).map(key => key);
            }
        },
        commitForm() {
